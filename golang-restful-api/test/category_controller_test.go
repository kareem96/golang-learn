...
func TestUpdateCategorySuccess(t *testing.T)  {
	...
	truncateCategory(db)
	tx, _ := db.Begin()
	categryReposiotry := repository.NewCategoryRepository()
	category := categryReposiotry.Save(context.Background(), tx, domain.Category{
		Name: "Gadget",
	})
	tx.Commit()
	router := setupRouter(db)
...
func TestListCategorySuccess(t *testing.T)  {
	...
	category1 := categryReposiotry.Save(context.Background(), tx, domain.Category{
		Name: "Computer",
	})
	tx.Commit()
	router := setupRouter(db)
